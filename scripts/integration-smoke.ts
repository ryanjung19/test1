...
async function main() {
  ...
  const approved = await reviewProspect({
    prospectId: ingested[0].id,
    status: "approved",
  });
  ...
  assert.equal(
    databaseConstraintBlockedOverlap,
    true,
    "PostgreSQL GiST exclusion constraint must reject direct overlapping writes",
  );
  ...
  await db
    .update(bookings)
    .set({ holdExpiresAt: new Date(Date.now() - oneHour) })
