...
커스텀 reverse proxy에서는 원래 요청의 Host를 보존하고 `X-Forwarded-Proto`를 실제 TLS 프로토콜로 덮어써야 합니다. 앱의 동일 출처 검사와 인증 콜백은 이 값을 사용하며 `X-Forwarded-Host`는 신뢰하지 않습니다. 프록시 뒤 원본 서버는 외부에서 직접 접근할 수 없도록 제한합니다.
