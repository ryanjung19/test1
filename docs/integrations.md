...
주문 단위의 환불과 계약 전체 해지는 구별합니다. 각 결제 주문은 별도 기간 이용권을 가지며, 선결제·다음 회차 실패·환불이 현재의 다른 정상 이용권을 덮어쓰거나 회수하지 않습니다. 주문 환불은 해당 이용권만 회수하고, 남은 이용권이 없으면 계약의 결제 상태를 past_due로 둡니다. 계약 전체 취소는 별도 운영 전이입니다. 지연·중복 이벤트의 순서는 주문별 검증 시각으로 판정하고 오래된 주문의 확인된 결제·환불 증빙도 보존합니다. 이미 종료된 계약의 결제가 뒤늦게 확인되면 증빙은 보관하지만 이용권은 발급하지 않으므로 운영자가 환불 여부를 확인해야 합니다.
